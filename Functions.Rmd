...
```{r, echo = FALSE}
df <- 
  data.frame(x1 = c("Addition", "Multiplication", "Composition"),
             x2 = c("$\\lbrack f+g\\rbrack(x)=:\\big\\{\\big(x,f(x)+g(x)\\big)\\mid x\\in dom(f)\\cap dom(g)\\big\\}$",
                    "$\\lbrack f\\cdot g\\rbrack(x):=\\big\\{\\big(x,f(x)\\cdot g(x)\\big) \\mid x\\in dom(f)\\cap dom(g)\\big\\}$",
                    "$\\lbrack f\\circ g\\rbrack(x)=\\big\\{\\big(x,f\\big(g(x)\\big) \\mid x\\in dom(g)$ and  $g(x)\\in dom(f)\\big\\}$"),
