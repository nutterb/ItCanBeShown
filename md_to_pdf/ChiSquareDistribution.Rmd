...
\chapter{The Chi-Square Distribution}
\label{ChiSquareDistribution_the-chi-square-distribution}
```{r, echo=FALSE}
library(ggplot2)
library(plyr)
...
\label{ChiSquareDistribution_pdf}
...
\label{ChiSquareDistribution_cumulative-distribution-function}
...
```{r, echo=FALSE, fig.cap='The graphs on the left and right depict the Chi-Square probability distribution and cumulative distribution functions, respectively, for $\\nu=4, 7, 10$.  As $\\nu$ gets larger, the distribution becomes flatter with thicker tails.', fig.height=3, fig.width=6}
Chi <- expand.grid(x=rep(seq(0, 20, by=.01), 2),
                   display=factor(c("PDF", "CDF"), c("PDF", "CDF")),
                   nu = c(4, 7, 10))
Chi <- transform(Chi, 
                 p = ifelse(display %in% "PDF",
                            dchisq(x, nu),
                            pchisq(x, nu)))
ggplot(Chi, aes(x=x, y=p, colour=factor(nu))) + geom_line() + 
  facet_grid(~ display) + 
  scale_colour_manual(values=greens.pal[c(1, 3, 6)]) + 
  xlab("x") + ylab("Probability") + 
  labs(colour=bquote(nu))
```

\section{Expected Values}
\label{ChiSquareDistribution_expected-values}
$$
\begin{aligned}
E(X)
...
\color{red}{\text{$\int\limits_{0}^{\infty}x^{\alpha-1}e^{-\frac{x}{\beta}}dx
...
\label{ChiSquareDistribution_moment-generating-function}
$$
\begin{aligned}
M_X(t)
	&= E(e^{tX})\\
...
\label{ChiSquareDistribution_maximum-likelihood-function}
Let $x_1,x_2,\ldots,x_n$ be a random sample from a Chi-square distribution with parameter $\nu$.

\subsection{Likelihood Function}
\label{ChiSquareDistribution_likelihood-function}
$$
\begin{aligned}
L(\theta)
...
\label{ChiSquareDistribution_log-likelihood-function}
$$
\begin{aligned}
\ell(\theta)
	&= \ln\big(L(\theta)\big)\\
...
\subsection{MLE for $\mathbf{\nu}$}
\label{ChiSquareDistribution_mle-for-mathbfnu}
$$
\begin{aligned}
\frac{d\ell}{d\nu}
	&= -\frac{n}{2} \ln 2 
...
0 &= -\frac{n}{2} \ln 2
		- \frac{n}{2\Gamma\big(\frac{\nu}{2}\big)} \Gamma^\prime\Big(\frac{\nu}{2}\Big)
		+ \frac{1}{2} \sum\limits_{i=1}^{n}\ln x_i\\
\frac{n}{2} \ln 2 - \frac{1}{2}\sum\limits_{i=1}^{n}\ln x_i
		&= -\frac{n}{2\Gamma\big(\frac{\nu}{2}\big)} \Gamma^\prime\Big(\frac{\nu}{2}\Big)\\
n\ln 2 - \sum\limits_{i=1}^{n}\ln x_i
		&= -\frac{n}{\Gamma\big(\frac{\nu}{2}\big)} \Gamma^\prime\Big(\frac{\nu}{2}\Big)\\
\frac{\sum\limits_{i=1}^{n}\ln x_i - n\ln 2}{n}
...
\section{Theorems for the Chi-Square Distribution}
\label{ChiSquareDistribution_theorems-for-the-chi-square-distribution}

\subsection{Validity of the Distribution}
\label{ChiSquareDistribution_validity-of-the-distribution}
$$
...
\subsection{Sum of Chi-Square Random Variables}
\label{ChiSquareDistribution_sum-of-chi-square-random-variables}

Let $X_1 , X_2 , \ldots , X_n$ be independent Chi-Square random variables with parameter $\nu_i$, that is $X_i\sim\chi^2(\nu_i),\ i=1,2,\ldots,n$.

Suppose $Y = \sum\limits_{i=1}^{n}X_i$.

Then $Y\sim\chi^2(\sum\limits_{i=1}^{n}\nu_i)$.
...
Thus $Y\sim\chi^2\bigg(\sum\limits_{i=1}^{n}\nu_i\bigg)$. \rule{.05in}{.05in}


\subsection{Square of a Standard Normal Random Variable}
\label{ChiSquareDistribution_square-of-a-standard-normal-random-variable}
...
\color{red}{\text{$\int\limits_{-\infty}^{\infty}f(x)dx
		= 2\int\limits_{0}^{\infty}f(x)dx$}} &\\
\color{red}{\text{when $f(x)$ is an even function (\ref{Integration1.2})}} &\\
  &= \frac{2}{\sqrt{2\pi}}\int\limits_{0}^{\infty}
		e^{-\frac{z^2}{2}(1-2t)}dz\\
\color{red}{\text{Let $u=\frac{z^2}{2}(1-2t)$}} &\\
\color{red}{\text{$\Rightarrow z =\frac{\sqrt{2}u^{\frac{1}{2}}}{(1-2t)^{\frac{1}{2}}}$}} &\\
\color{red}{\text{So $dz=\frac{\sqrt{2}u^{-\frac{1}{2}}} {2(1-2t)^{\frac{1}{2}}}$}} &\\
