...
Let $A	
	= \displaystyle \int\limits_{-\infty}^{\infty}\frac{1}{\sqrt{2\pi}\sigma}e^\frac{-(x-\mu)^2}{2\sigma^2}dx$.  
By using the identity transformation $y=x$, we may also write 
$A
	= \displaystyle \int\limits_{-\infty}^{\infty}\frac{1}{\sqrt{2\pi}\sigma}e^\frac{-(y-\mu)^2}{2\sigma^2}dy\\
\Rightarrow A^2 = \int\limits_{-\infty}^{\infty}\frac{1}{\sqrt{2\pi}\sigma}
 ...
 = \footnote[1]{Substitute $t=x-\mu$ and $u=y-\mu$}
	 \ \ \frac{1}{2\pi\sigma^2}\int\limits_{-\infty}^{\infty}\int\limits_{-\infty}^{\infty}
		e^{-\frac{1}{2\sigma^2}t^2+u^2}dt\ du
	= \footnote[2]{$t=r\cos\theta$ and $u=r\sin\theta$.\\ Thus 
		$t^2+u^2=r^2\cos^2\theta+r^2\sin^2\theta=r^2(\cos^2\theta+\sin^2\theta)=r^2$}
	\ \ \frac{1}{2\pi\sigma^2}\int\limits_{0}^{2\pi}\int\limits_{0}^{\infty}
...
\\
Thus  $\displaystyle \int\limits_{-\infty}^{\infty}\frac{1}{\sqrt{2\pi}\sigma}e^\frac{-(x-\mu)^2}{2\sigma^2}dx=1$\ \rule{.05in}{.05in}


%New Subsection
\subsection{Validity of the Distribution (Gamma Function)}
\label{Normal4.2}
$\displaystyle \int\limits_{-\infty}^{\infty}\frac{1}{\sqrt{2\pi}\sigma}e^\frac{-(x-\mu)^2}{2\sigma^2}dx=1$.\\
\\
\it Proof: \rm\\
\\
$\displaystyle \int\limits_{-\infty}^{\infty}\frac{1}{\sqrt{2\pi}\sigma}e^\frac{-(x-\mu)^2}{2\sigma^2}dx
	=\frac{1}{\sqrt{2\pi}\sigma}\int\limits_{-\infty}^{\infty}e^\frac{-(x-\mu)^2}{2\sigma^2}dx
	=\footnote[1]{$y=x-\mu\ \Rightarrow x=y+\mu\ \Rightarrow dx=dy$}
	\ \ \frac{1}{\sqrt{2\pi}\sigma}\int\limits_{-\infty}^{\infty}e^\frac{-(y)^2}{2\sigma^2}dy\\
=\frac{1}{\sqrt{2\pi}\sigma}\int\limits_{-\infty}^{\infty}e^{-\frac{1}{2}\frac{y^2}{\sigma^2}}dy
	=\footnote[2]{$u=(\frac{y}{\sigma})^2\ \Rightarrow y=\sigma u^\frac{1}{2}\ 
		\Rightarrow dy=\frac{1}{2}\sigma u^{-\frac{1}{2}}$}
	\ \ \frac{1}{\sqrt{2\pi}\sigma}\int\limits_{-\infty}^{\infty}e^{-\frac{u}{2}}\frac{1}{2}\sigma u^{-\frac{1}{2}}du\\
=\frac{\sigma}{2\sqrt{2\pi}\sigma}\int\limits_{-\infty}^{\infty}u^{-\frac{1}{2}}e^{-\frac{u}{2}}du
	=\footnote[3]{If $f(x)$ is an even function then $\int\limits_{-\infty}^{\infty}f(x)dx
		=2\int\limits_{0}^{\infty}f(x)dx$ (Theorem \ref{Integration1.2}).}
	\ \ \frac{2\sigma}{2\sqrt{2\pi}\sigma}\int\limits_{0}^{\infty}u^{\frac{1}{2}-1}e^{-\frac{u}{2}}du\\
=\footnote[4]{$\int\limits_{0}^{\infty}x^{\alpha-1}e^{-\frac{x}{\beta}}dx
		=\beta^\alpha\Gamma(\alpha)$}
	\ \ \frac{1}{\sqrt{2\pi}}2^\frac{1}{2}\Gamma\big(\frac{1}{2}\big)
	=\footnote[5]{$\Gamma(\frac{1}{2})=\sqrt{\pi}$}
	\ \ \frac{\sqrt{2\pi}}{\sqrt{2\pi}}=1$ \rule{.05in}{.05in}



%New Subsection
\subsection{Multiple of a Normal Random Variable}
\label{Normal4.3}
...
\it Proof: \rm \\
\\
$M_Y(t)=E(e^{tY})=E(e^{tcX})=\exp\Big\{\mu tc+\frac{t^2c^2\sigma^2}{2}\Big\}=\exp\Big\{c\mu t+\frac{t^2c^2\sigma^2}{2}\Big\}$\\
\\
Which is the Moment Generating Function of a Normal random variable with mean $c\mu$ and variance $c^2\sigma^2$.  Thus $Y\sim$Normal$(c\mu,\ c^2\sigma^2)$. \rule{.05in}{.05in}

%New Subsection
\newpage
\subsection{Weighted Sum of Normal Random Variables}
\label{Normal4.4}
...
\it Proof: \rm \\
\\
$\displaystyle M_Y(t)=E(e^{tY})=E(e^{t(a_1X_1+a_2X_2+\cdots+a_nX_n})=E(e^{ta_1X_1}e^{ta_2X_2}\cdots e^{ta_nX_n})\\\\
...
=\exp\Big\{t\sum\limits_{i=1}^{n}a_i\mu_i+\frac{t\sum\limits_{i=1}^{n}a_i^2\sigma_i^2}{2}\Big\}$\\
\\
Which is the mgf of a Normal random variable with parameters $\sum\limits_{i=1}^{n}a_i\mu_i$ and $\sum\limits_{i=1}^{n}a_i^2\sigma_i^2$.  Thus $Y\sim$Normal$\big(\sum\limits_{i=1}^{n}a_i\mu_i,\ \sum\limits_{i=1}^{n}a_i^2\sigma_i^2\big)$. \rule{.05in}{.05in}
...
\it Proof: \rm \\
\\
$Y=\sum\limits_{i=1}^{n}X_i$ is a special case of the Weighted Sum of Normal Random Variables where each of the weights is equal to 1.  It follows then that $Y\sim$Normal$\big(\sum\limits_{i=1}^{n}1\mu_i,\ \sum\limits_{i=1}^{n}1^2\sigma_i^2\big)$ which is equivalent to saying $Y\sim$Normal$\big(\sum\limits_{i=1}^{n}\mu_i,\ \sum\limits_{i=1}^{n}\sigma_i^2\big)$. \rule{.05in}{.05in}


%New Subsection
\newpage
\subsection{Standard Normal Transformation}
\label{Normal4.6}
Let $X$ be a Normal random variable with mean $\mu$ and variance $\sigma^2$.  That is, $X\sim$Normal($\mu,\sigma^2$).  If $Z=\frac{X-\mu}{\sigma}$, then $Z\sim$Normal(0,1), and we say $Z$ has a \it Standard Normal \rm distribution.\\
\\
...
$\displaystyle E(e^{tZ})=E\Big(\exp\Big\{t\frac{x-\mu}{\sigma}\Big\}\Big)=E\Big(\exp\Big\{\frac{tx-t\mu}{\sigma}\Big\}\Big)
...
\\
$\displaystyle E(e^{tX})=E(e^{t(z\sigma+\mu)})=E(e^{tz\sigma}e^{t\mu})=e^{t\mu}E(e^{t\sigma z})\\
=\exp\{t\mu\}\exp\Big\{\frac{t^2\sigma^2}{2}\Big\}=\exp\Big\{\mu t+\frac{t^2\sigma^2}{2}\Big\}$\\
\\
Which is the Moment Generating Funciton of a Normal random variable with mean $\mu$ and variance $\sigma^2$.  Thus $X\sim$Normal($\mu,\sigma^2$).\ \ \rule{.05in}{.05in}


%New Subsection
\subsection{Distribution of $\bar X$}
\label{Normal4.7}
...
$\displaystyle M_{\bar X}(t)=exp\Big(t\sum\limits_{i=1}^{n}\frac{1}{n}\mu
...
Which is the mgf of a Normal random variable with parameters $\mu$ and $\frac{\sigma^2}{n}$.  Thus $\bar X\sim$Normal$\big(\mu,\ \frac{\sigma^2}{n}\big)$. \rule{.05in}{.05in}

%New Subsection
\subsection{Square of a Standard Normal Random Variable}
\label{Normal4.8}
	\markboth{Normal Distribution}{Probability Distributions and Theory}
If $Z\sim N(0,1)$, then $Z^2\sim\chi^2(1)$.\\
\\
\it Proof: \rm\\
$\displaystyle M_{Z^2}(t)=E(e^{tZ^2})=\int\limits_{-\infty}^{\infty}e^{tz^2}\frac{1}{\sqrt{2\pi}}
		e^{-\frac{z^2}{2}}dz
...
=\footnote[1]{$\int\limits_{-\infty}^{\infty}f(x)dx
		=2\int\limits_{0}^{\infty}f(x)dx$ when $f(x)$ is an even function (Theorem \ref{Integration1.2}).}
	\ \ \frac{2}{\sqrt{2\pi}}\int\limits_{0}^{\infty}
		e^{-\frac{z^2}{2}(1-2t)}dz
	=\footnote[2]{Let $u=\frac{z^2}{2}(1-2t)\ \Rightarrow z=\frac{\sqrt{2}u^{\frac{1}{2}}}
		{(1-2t)^{\frac{1}{2}}}$
	 So $dz=\frac{\sqrt{2}u^{-\frac{1}{2}}} {2(1-2t)^{\frac{1}{2}}}$}
	\ \ \frac{2}{\sqrt{2\pi}}\int\limits_{0}^{\infty}e^{-u}
		\frac{\sqrt{2}u^{-\frac{1}{2}}}{2(1-2t)^{\frac{1}{2}}}du\\
=\frac{2\sqrt{2}}{2\sqrt{2\pi}(1-2t)^{\frac{1}{2}}}
		\int\limits_{0}^{\infty}e^{-u}u^{-\frac{1}{2}}du
	=\frac{2\sqrt{2}}{2\sqrt{2\pi}(1-2t)^{\frac{1}{2}}}
		\int\limits_{0}^{\infty}u^{\frac{1}{2}-1}due^{-u}u^{-\frac{1}{2}}du\\
\footnote[3]{$\int\limits_{0}^{\infty}x^{\alpha-1}e^{-\frac{x}{\beta}}dx
		=\beta^\alpha\Gamma(\alpha)$}
	\ \ \frac{1}{\sqrt{\pi}(1-2t)^{\frac{1}{2}}}\Gamma(\frac{1}{2})
	=\frac{\sqrt{\pi}}{\sqrt{\pi}(1-2t)^{\frac{1}{2}}}
	=\frac{1}{(1-2t)^{\frac{1}{2}}}=(1-2t)^{-\frac{1}{2}}\\$
\\
Which is the mgf of a Chi-Square random variable with 1 degree of freedom.  Thus $Z^2\sim\chi^2(1)$. \rule{.05in}{.05in} 
