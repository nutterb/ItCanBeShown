...
$$
SS_{Factor}
	= \sum\limits_{i=1}^{a} n_i(\bar x_{i+} - \bar x_{++})^2$ and 
$$
...
Using the theorems in Chapter \ref{Summation} it is can be shown that each of the three cross products is equal to zero.  However, the physical tedium of reducing each cross product is much greater than the approach taken above.
