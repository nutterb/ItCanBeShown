...
Let $X$ be a random variable following a binomial distribution with parameters $n$ and $p$. Let $x$ be the observed frequency of experimental subjects exhibiting the trait of interest. 
...
Reject $H_0$ if $x < Binomial(\alpha/2, n, \pi)$ or $x > Binomial(1 - \alpha/2, n, \pi)$

```{r, echo = FALSE, fig.path = 'figures/', fig.cap='The examples displayed use $n = 20$. For the top, middle, and bottom examples, $p$ is set at 0.3, 0.5, and 0.75, respectively. Notice that in some cases, the rejection regions for $\\alpha = 0.10$ and $\\alpha = 0.05$ are identical.'}
DF <- 
  bind_rows(
    data.frame(x = 0:20,
               n = 20,
...
Reject $H_0$ when $x \leq Binomial(\alpha, n, \pi)$

For an upper tailed test, the decision rule is:

Reject $H_0$ when $x \geq Binomial(1 - \alpha, n, \pi)$

```{r, echo = FALSE, fig.path = 'figures/', fig.cap='The examples displayed use $n = 20$. For the top, middle, and bottom examples, $p$ is set at 0.3, 0.5, and 0.75, respectively.'}
DF <- 
  bind_rows(
    data.frame(x = 0:20,
               n = 20,
...
\gamma(\pi_a) &= P_{\pi_a}(x \in C) \\
 &= P_{\pi_a}(x \leq Binomial(\alpha / 2, n, \pi_a)) + \\
 & \ \ \ \ \ \ \ P_{\pi_a}(x \geq Binomial(1 - \alpha / 2, n, \pi_a))
\end{align}$$
...
$$\begin{align}
\gamma(\pi_a) &= P_{\pi_a}(x \in C) \\
 &= P_{\pi_a}(x \leq Binomial(\alpha, n, \pi_a)) 
\end{align}$$
...
$$\begin{align}
\gamma(\pi_a) &= P_{\pi_a}(x \in C) \\
 &= P_{\pi_a}(x \geq Binomial(1 - \alpha, n, \pi_a)) 
\end{align}$$
